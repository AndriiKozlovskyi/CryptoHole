  ...
  <div class="flex flex-col gap-y-1 h-full w-full justify-center items-center" @keydown.enter="tryLogin">
    ...
    >
			...
			<div class="flex flex-col">
				<div class="flex flex-col">
					<p :class="`text-red-500 text-[12px] apple-font px-3 ${identifierEmpty ? 'text-opacity-100' : 'text-transparent'}`" >* Username cannot be empty</p>
					<MyInput v-model="credentials.identifier" placeholder="Username or email" @focusin="identifierEmpty = false"/>
				</div>
				<div class="flex flex-col">
					<p :class="`text-red-500 text-[12px] apple-font px-3 ${passwordEmpty ? 'text-opacity-100' : 'text-transparent'}`" >* Password cannot be empty</p>
