<template>
  <div class="h-full w-full flex justify-center items-center">
    <div
      class="bg-background-color justify-center items-center flex w-[22rem] h-[39rem] rounded-2xl flex-col space-y-2 relative"
    >
      <div class="flex flex-row items-center justify-center mb-8">
        <button
          class="pi pi-arrow-left text-[#4619bd] text-[1.3rem] hover:text-[#4c12b2] duration-300 cursor-pointer absolute left-[0rem] flex self-center"
          @click="goToAdminPage"
        ></button>
        <h1 class="text-white text-2xl font-semiboldn apple-font">Create a project</h1>
      </div>
      <MyInput
        class="w-[20rem] h-[2.5rem] rounded-md"
        placeholder="Name"
        type="text"
        v-model="name"
      />
      <MultiSelect
        ...
        :maxSelectedLabels="3"
        class="text-white bg-primary-item-color w-full"
      />
      <MyInput
        class="w-[20rem] h-[2.5rem] rounded-md"
      ...
      <div class="flex flex-row justify-between w-full">
        <button
          label="Clear"
          class="rounded-lg hover:bg-gray-600 px-6 py-2 apple-font bg-hover-primary-item-color text-white"
          @click="resetForm"
        >
          Clear
        </button>
        <button
          label="Save"
          class="rounded-lg hover:bg-[#4619bd] px-6 py-2 apple-font bg-[#4c12b2] text-white"
          @click="createProject"
        >
          Save
        </button>
...
const createProject = () => {
  ...
  if (!numberPattern.test(expenses.value)) {
    ToastManager.showErrorToast(
      toast,
      'Invalid cost for one account. Please, provide a valid number.'
    )
    return
  }
